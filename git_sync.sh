#!/bin/bash
# Taiwan Earthquake Tracker - PythonAnywhere Git Sync Automation
...
echo "=== Earthquake Tracker Git Sync Start: $(date) ==="

# 1. Execute the Python crawler to get the latest earthquake data
...
if [ -n "$STATUS" ]; then
    echo "[*] Changes detected in earthquake data or README. Preparing to commit..."
    
    # Stage the earthquake data and the updated README
    git add data/ README.md
    
    # Commit with local timestamp
    git commit -m "Auto-update earthquake data and README: $(date +'%Y-%m-%d %H:%M:%S')"
    ...
    echo "[*] No changes detected in earthquake data. Skipping git push."
fi

echo "=== Earthquake Tracker Git Sync Complete: $(date) ==="
echo ""
